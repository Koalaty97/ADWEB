...
const InkomstenUitgavenGraph: React.FC<InkomstenUitgavenGraphParameters> = ({ huishoudboekjeId, maand }) => {
    const qFn = useCallback(
        (ref: any) => 
            query(ref, where("huishoudboekjeId", "==", huishoudboekjeId),
                       where("maand", "==", maand)), [maand]);

    const { items: inkomsten } = useCollection<Inkomst>(db, collectionInkomsten, qFn);
    const { items: uitgaven } = useCollection<Uitgave>(db, collectionUitgaven, qFn);

    const data = useMemo(
        () => dailySeries(
            mapInkomstenToTransactions(inkomsten), 
            mapUitgavenToTransactions(uitgaven), new Date().getFullYear(), maand),
    ...
    return(
        ...
        <LineChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <XAxis dataKey="date" tickFormatter={(str) => str.slice(-2)} interval={Math.floor(data.length / 7)} />
          <YAxis tickFormatter={(n) => n.toLocaleString('nl-NL', { style: 'currency', currency: 'EUR' })} />
          <Tooltip 
          ...
          <Line type="monotone" dataKey="income" name="Inkomen" stroke="#4caf50" dot={false} strokeWidth={2} />
          <Line type="monotone" dataKey="expense" name="Uitgaven" stroke="#f44336" dot={false} strokeWidth={2}
          />
        </LineChart>
      </ResponsiveContainer>
    )
