...
export function InkomenOverview({ huishoudboekjeId, isOwner, maand }: InkomenOverviewParameters) {
  const { inkomsten, loading, error: loadError, formError, add } = useInkomenData(
    huishoudboekjeId,
    maand
  );
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      <IncomeTable inkomsten={inkomsten} categorieen={categorieen} isOwner={isOwner} />
      <AddInkomstModal
        open={modalOpen}
