...
export function UitgavenOverview({ huishoudboekjeId, isOwner, maand }: any) {
  const { uitgaven, loading, loadError, formError, add } = useUitgavenData(huishoudboekjeId, maand);
  ...
  return (
    <div style={{ width: "100%" }}>
      <Box display="flex" justifyContent="space-between" mb={2}>
        ...
        <Button variant="contained" onClick={() => setModalOpen(true)}>Toevoegen</Button>
      </Box>
      <UitgavenTable uitgaven={uitgaven} categorieen={categorieen} isOwner={isOwner} />
      <AddUitgaveModal
        open={modalOpen}
