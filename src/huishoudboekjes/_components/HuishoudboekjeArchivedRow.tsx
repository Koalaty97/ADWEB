...
export function HuishoudboekjeArchivedRow({ entry }: { entry: Huishoudboekje }) 
{
    const { user } = useAuth();
    ...
    return(
        <TableRow key={entry.naam}>
            ...
            <TableCell>
                <Button sx={{ alignSelf: 'anchor-center', mr: '10px' }} variant="contained" onClick={(_) => onArchiveer(entry)}>Activeren</Button>
            </TableCell>
        </TableRow>
    );
}