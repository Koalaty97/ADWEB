...
const CategorieUitgavenGraph: React.FC<CategorieUitgavenGraphParameters> = ({ huishoudboekjeId }) => {
    const { items: categorieen } = useCategorieen(huishoudboekjeId);
    const categorieIds = useMemo(() => categorieen.map(cat => cat.id), [categorieen]);
