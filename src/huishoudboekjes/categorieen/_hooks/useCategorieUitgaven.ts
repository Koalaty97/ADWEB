...
export function useCategorieUitgaven(categorieen: Categorie[], uitgaven: Uitgave[]) : CategorieUitgaven[]
{
    return categorieen.map<CategorieUitgaven>(categorie => ({
        ...
        uitgave: uitgaven.filter(uitgave => uitgave.categorieId == categorie.id)
