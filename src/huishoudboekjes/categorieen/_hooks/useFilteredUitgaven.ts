import { query, where, CollectionReference, Query, documentId } from 'firebase/firestore';
import { Uitgave } from '../../../models/Uitgave';
import { useCollection } from '../../../hooks/useCollection';
import { collectionUitgaven, db } from '../../../firebase';
import { useCallback } from 'react';

function useFilteredUitgaven(
  categorieIds: string[]
) {
  ...
  const { items: rawUitgaven, loading, error } = useCollection<Uitgave>(
    db,
    collectionUitgaven,
    queryUitgaven
  );
